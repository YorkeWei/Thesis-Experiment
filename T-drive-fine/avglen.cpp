...
void database_loader(std::vector<trajectory>& trajectory_list)
{
	std::string in_path = "./T-drive-query-2/";
...
int main()
{
	...
	database_loader(trajectory_list);
	int trac_len = trajectory_list.size();
	int i;
	double avglen = 0.0;
	for (i = 0; i < trac_len; i++)
	{
		avglen += (((double)(trajectory_list[i].size())) / ((double)trac_len));
	}
	std::cout << avglen << std::endl;
	return 0;
}
