...
#ifndef OURS_PIVOT_HPP
#define OURS_PIVOT_HPP
...
void get_index_segments(const trajectory& raw_trac, std::vector<segment>& index_segments, const std::string& strategy)
{
    int len = raw_trac.size();
    std::vector<std::pair<double, int>> point_weight;
    int i;
    if (strategy == "Neighbor Distance")
    {
        for (i = 1; i < len; i++)
        {
            double weight = euclid_dist(raw_trac[i - 1], raw_trac[i]);
    ...
    for (i = 0; i < K; i++)
    {
        index_segments.emplace_back(std::make_pair(raw_trac[order_index[i] - 1], raw_trac[order_index[i]]));
    }
}

void get_index_segments_list(const std::vector<trajectory>& trajectory_list, std::vector<std::vector<segment>>& index_segments_list, const std::string& strategy)
{
    int len = trajectory_list.size();
    int i;
    for (i = 0; i < len; i++)
    {
        const trajectory& raw_trac = trajectory_list[i];
        std::vector<segment> index_segments;
        get_index_segments(raw_trac, index_segments, strategy);
        index_segments_list.emplace_back(index_segments);
    }
}

#endif //OURS_PIVOT_HPP
