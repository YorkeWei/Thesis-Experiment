...
#ifndef DITA_DIST_HPP
#define DITA_DIST_HPP
...
//bool dou_dir_Frechet_verify(const trajectory& t1, const trajectory& t2, double threshold)
//{
//    int t1_len = t1.size();
//    int t2_len = t2.size();
//    //判断t1或t2的长度是否为0，正常情况下不会出现
...
//    {
//        Frechet_mat[i][0] = 0.0;
//        Frechet_mat[t1_len - 1 - i][t2_len - 1] = 0.0;
//        for (j = 0; j <= i; j++)
//        {
//            Frechet_mat[i][0] = std::max(euclid_dist(t1[j], t2[0]), Frechet_mat[i][0]);
//            Frechet_mat[t1_len - 1 - i][t2_len - 1] = std::max(euclid_dist(t1[t1_len - 1 - j], t2[t2_len - 1]), Frechet_mat[t1_len - 1 - i][t2_len - 1]);
//            if (Frechet_mat[i][0] > threshold)
//            {
//                return false;
//            }
//            if (t1_len - 1 - i == i && t2_len - 1 == 0)
//            {
//                return Frechet_mat[i][0] <= threshold;
//            }
//        }
//    }
//    for (i = 0; i < t2_len; i++)
//    {
//        Frechet_mat[0][i] = 0.0;
//        Frechet_mat[t1_len - 1][t2_len - 1 - i] = 0.0;
//        for (j = 0; j <= i; j++)
//        {
//            Frechet_mat[0][i] = std::max(euclid_dist(t1[0], t2[j]), Frechet_mat[0][i]);
//            Frechet_mat[t1_len - 1][t2_len - 1 - i] = std::max(euclid_dist(t1[t1_len - 1], t2[t2_len - 1 - j]), Frechet_mat[t1_len - 1][t2_len - 1 - i]);
//            if (0 == t1_len - 1 && i == t2_len - 1 - i)
//            {
//                return Frechet_mat[0][i];
//            }
//        }
//    }
//    for (i = 1; i < t1_len - 1; i++)
//    {
//        for (j = 1; j < t2_len - 1; j++)
//        {
//            Frechet_mat[i][j] = std::max(euclid_dist(t1[i], t2[j]), std::min(std::min(Frechet_mat[i - 1][j], Frechet_mat[i - 1][j - 1]), Frechet_mat[i][j - 1]));
//            Frechet_mat[t1_len - 1 - i][t2_len - 1 - j] = std::max(euclid_dist(t1[t1_len - 1 - i], t2[t2_len - 1 - j]), std::min(std::min(Frechet_mat[t1_len - i][t2_len - 1 - j], Frechet_mat[t1_len - i][t2_len - j]), Frechet_mat[t1_len - i - 1][t2_len - j]));
//
//        }
//    }
//    return Frechet_mat[t1_len - 1][t2_len - 1];
